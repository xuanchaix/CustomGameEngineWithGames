#include "Game/Aries.hpp"
#include "Game/GameCommon.hpp"
#include "Game/Game.hpp"
#include "Game/Ray.hpp"
#include "Game/Map.hpp"
#include "Game/Bullet.hpp"
#include "Game/App.hpp"

Aries::Aries( Vec2 const& startPos, Map* map, EntityFaction faction )
	:Entity( startPos, map )
{
	m_speed = g_gameConfigBlackboard.GetValue( "ariesDriveSpeed", 0.7f );
	m_velocity = Vec2( 0, 0 );
	m_orientationDegrees = g_theGame->m_randNumGen->RollRandomFloatInRange( 0.f, 360.f );
	m_angularVelocity = g_gameConfigBlackboard.GetValue( "ariesTurnRate", 120.f );

	m_physicsRadius = ARIES_PHYSICS_RADIUS;
	m_cosmeticRadius = ARIES_COSMETIC_RADIUS;

	m_maxHealth = g_gameConfigBlackboard.GetValue( "ariesMaxHealth", 5.f );
	m_health = m_maxHealth;

	m_type = EntityType::_ARIES;
	m_faction = faction;

	m_target = m_map->GetNearestEnemyActor( this );
	m_curState = 1;

	m_wonderingCoolDown = g_gameConfigBlackboard.GetValue( "ariesWanderRandomizePeriod", 1.f );
	m_wonderingTime = m_wonderingCoolDown;

	m_isPushedByEntities = true;
	m_doesPushEntities = true;
	m_isPushedByWalls = true;
	m_isHitByBullets = true;
	m_isActor = true;
	m_isRanged = false;
	
	if (m_faction == EntityFaction::FACTION_EVIL) {
		m_tankTexture = g_theRenderer->CreateOrGetTextureFromFile( "Data/Images/EnemyAries.png" );
	}
	else if (m_faction == EntityFaction::FACTION_GOOD) {
		m_tankTexture = g_theRenderer->CreateOrGetTextureFromFile( "Data/Images/FriendlyTank2.png" );
	}

	m_driveHalfAngleDegrees = g_gameConfigBlackboard.GetValue( "ariesDriveHalfAperture", 45.f );

	Entity::InitializeMovingWarrior();
}

Aries::~Aries()
{

}

void Aries::Update( float deltaTime )
{
	if (m_isDead) {
		m_isGarbage = true;
		return;
	}
	if (m_health <= 0.f) {
		Die();
		return;
	}

	Entity::ConductAIMovingWarrior( deltaTime );
}

void Aries::Render() const
{
	if (m_isDead) {
		return;
	}
	std::vector<Vertex_PCU> tankVerts;
	tankVerts.reserve( 6 );
	AddVertsForOBB2D( tankVerts, OBB2( m_position, Vec2::MakeFromPolarDegrees( m_orientationDegrees ), Vec2( m_cosmeticRadius, m_cosmeticRadius ) ), Rgba8( 255, 255, 255, 255 ) );
	g_theRenderer->BindTexture( m_tankTexture );
	g_theRenderer->SetModelConstants();
	g_theRenderer->DrawVertexArray( tankVerts );
}

void Aries::Die()
{
	m_map->PlaySound( AudioName::EnemyDied, m_position );
	m_isDead = true;
	m_map->SpawnNewEntity( EntityType::_EXPLOSION, m_position, m_faction, this );
}

void Aries::DebugRender() const
{
	std::vector<Vertex_PCU> verts;
	verts.reserve( 400 );
	AddVertsForDebugDrawLine( verts, m_position, m_orientationDegrees, m_cosmeticRadius, 0.02f, Rgba8( 255, 0, 0, 255 ) );
	AddVertsForDebugDrawLine( verts, m_position, m_orientationDegrees + 90.f, m_cosmeticRadius, 0.02f, Rgba8( 0, 255, 0, 255 ) );
	AddVertsForDebugDrawRing( verts, m_position, m_cosmeticRadius, 0.04f, Rgba8( 255, 0, 255, 255 ) );
	AddVertsForDebugDrawRing( verts, m_position, m_physicsRadius, 0.04f, Rgba8( 0, 255, 255, 255 ) );
	AddVertsForDebugDrawLine( verts, m_position, Atan2Degrees( m_velocity.y, m_velocity.x ), m_velocity.GetLength(), 0.02f, Rgba8( 255, 255, 0, 255 ) );

	AddVertsForDebugDrawLine( verts, m_position, m_lastSeenPosition, 0.1f, Rgba8( 0, 0, 0 ) );
	AddVertsForDebugDrawLine( verts, m_position, m_nextWayPointPosition, 0.1f, Rgba8( 255, 0, 0, 255 ) );

	g_theRenderer->BindTexture( nullptr );
	g_theRenderer->SetModelConstants();
	g_theRenderer->DrawVertexArray( verts );
}

void Aries::GetAttackedByBullet( Bullet* b )
{
	Vec2 normalVector = b->m_position - m_position;
	if (abs( GetShortestAngularDispDegrees( normalVector.GetOrientationDegrees(), m_orientationDegrees ) ) <= 45.f) {
		PushDiscOutOfFixedDisc2D( b->m_position, b->m_physicsRadius, m_position, m_physicsRadius );
		m_map->PlaySound( AudioName::BulletRicochet, m_position );
		b->BounceOff( normalVector.GetNormalized() );
	}
	else {
		Entity::GetAttackedByBullet( b );
	}
}

void Aries::BeAttacked( float hit )
{
	m_map->PlaySound( AudioName::EnemyHit, m_position );
	m_health -= hit;
}

