...
void AddVertsForDebugDrawRing( std::vector<Vertex_PCU>& verts, Vec2 const& center, float radius, float thickness, Rgba8 const& color )
{
	...
	constexpr float RADIANS_PER_SIDE = DEGREES_PER_SIDE * PI / 180.f;
	for (int i = 0; i < NUM_SIDES; i++) {
		verts.emplace_back( center + Vec2( CosRadians( RADIANS_PER_SIDE * i ) * radius, SinRadians( RADIANS_PER_SIDE * i ) * radius ), color );
		verts.emplace_back( center + Vec2( CosRadians( RADIANS_PER_SIDE * ((i + 1) % NUM_SIDES) ) * radius, SinRadians( RADIANS_PER_SIDE * ((i + 1) % NUM_SIDES) ) * radius ), color );
		verts.emplace_back( center + Vec2( CosRadians( RADIANS_PER_SIDE * i ) * (radius + thickness), SinRadians( RADIANS_PER_SIDE * i ) * (radius + thickness) ), color );
		verts.emplace_back( center + Vec2( CosRadians( RADIANS_PER_SIDE * ((i + 1) % NUM_SIDES) ) * radius, SinRadians( RADIANS_PER_SIDE * ((i + 1) % NUM_SIDES) ) * radius ), color );
		verts.emplace_back( center + Vec2( CosRadians( RADIANS_PER_SIDE * i ) * (radius + thickness), SinRadians( RADIANS_PER_SIDE * i ) * (radius + thickness) ), color );
		verts.emplace_back( center + Vec2( CosRadians( RADIANS_PER_SIDE * ((i + 1) % NUM_SIDES) ) * (radius + thickness), SinRadians( RADIANS_PER_SIDE * ((i + 1) % NUM_SIDES) ) * (radius + thickness) ), color );
	}
}
...
{
	AddVertsForLineSegment2D( verts, startPos, startPos + Vec2::MakeFromPolarDegrees( orientation, length ), thickness, color );
}
...
{
	AddVertsForLineSegment2D( verts, startPos, endPos, thickness, color );
}
