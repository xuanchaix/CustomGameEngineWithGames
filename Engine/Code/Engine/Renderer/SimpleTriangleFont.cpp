...
// Note: indexes are offset by -32 from ASCII value, so space(32) is at [0] and A(65) is at [33]
...
//------------------------------------------------------------------------------------------------
void AddVertsForTextTriangles2D( std::vector<Vertex_PCU>& verts, const std::string& text, const Vec2& startMins, float cellHeight, const Rgba8& color, int maxText, float cellAspect, bool isFlipped, float spacingFraction )
{
...
//	verts.reserve( verts.size() + estimatedNumberOfNewVerts );
	...
	int numGlyphs = (int)text.length() < maxText ? (int)text.length() : maxText;
	for( int i = 0; i < numGlyphs; ++ i )
	{
		char glyph = text[i];
		if (glyph == '\n') {
			cellMins.x = startMins.x;
			cellMins.y -= (cellHeight + cellHeight * spacingFraction);
		}
		else {
