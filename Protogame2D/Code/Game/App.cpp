...
// 
// 	//Vertex_PCU startIconVerts[3] = {
// 	//	Vertex_PCU( Vec3( -0.4f, -0.3f, 0 ), Rgba8( 0, 255, 0, (unsigned char)(255 * a) ), Vec2( 0, 0 ) ),
// 	//	Vertex_PCU( Vec3( -0.4f, 0.3f, 0 ), Rgba8( 0, 255, 0, (unsigned char)(255 * a) ), Vec2( 0, 0 ) ),
// 	//	Vertex_PCU( Vec3( 0.5f, 0.f, 0 ), Rgba8( 0, 255, 0, (unsigned char)(255 * a) ), Vec2( 0, 0 ) ),
// 	//};
// 
...
// 	g_theRenderer->DrawVertexArray( 3, startIconVerts );
	...
	static ConstantBuffer* constantBuffer = g_theRenderer->CreateConstantBuffer( sizeof( FontRNGConstants ) );
	g_theRenderer->BindConstantBuffer( 10, constantBuffer );
	FontRNGConstants FRNGConstants;
	FRNGConstants.m_bottomY = 750.f;
	FRNGConstants.m_time = Clock::GetSystemClock()->GetTotalSeconds();

	//SpriteAnimDefinition* anim = m_animDictionary[(int)AnimationName::Explosion];
	std::vector<Vertex_PCU> textVerts;
	textVerts.reserve( 1000 );

	g_ASCIIFont->AddVertsForTextInBox2D( textVerts, AABB2( Vec2(0.f, 0.f), Vec2(600.f, 300.f) ), 80.f, std::string( "jdscnsjc\nnewcnewkcnkewnckjewnckjewn\nkew" ), Rgba8::WHITE, 1.f, Vec2(1.f, 1.f), TextBoxMode::SHRINK_TO_FIT );
	g_ASCIIFont->AddVertsForTextInBox2D( textVerts, AABB2( Vec2(), Vec2( 1600.f, 800.f ) ), 100.f, "It's nice to have options!", Rgba8::WHITE, 1.f );
	g_theRenderer->BindTexture( &g_ASCIIFont->GetTexture(), 0 );
	g_theRenderer->BindTexture( g_theRenderer->CreateOrGetTextureFromFile( "Data/Images/PerlinNoiseTexture1.png" ), 1 );
	g_theRenderer->SetSamplerMode( SamplerMode::BILINEAR_CLAMP );
	g_theRenderer->SetCustomConstantBuffer( constantBuffer, &FRNGConstants, sizeof( FontRNGConstants ), 10 );
	g_theRenderer->BindShader( g_theRenderer->CreateShader( "Data/Shaders/FancyFont", VertexType::PCU ) );
...
{
	g_ASCIIFont = g_theRenderer->CreateOrGetBitmapFontFromFile( "Data/Fonts/SimpleFont", BitmapFontType::FntType );
}
