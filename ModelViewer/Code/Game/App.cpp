...
void App::Startup() {
	...
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "W: Forward S: Backward A: Left D: Right" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "Mouse move: Camera Rotation" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "Q/E: Move world up/down Shift: Speed * 10" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "Arrow Keys: Change sun direction" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "</>: Change sun intensity" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "[/]: Cycle Scene" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "L: Load Model File" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "R: Toggle Model Rotation" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "N: Toggle Model Rotation" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "1: Render Ambient" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "2: Render Diffuse" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "3: Render Specular" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "4: Render Emissive" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "5: Use Diffuse Map" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "6: Use Normal Map" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "7: Use Specular Map" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "8: Use Glossiness Map" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "9: Use Emissive Map" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "P: Pause" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "~: Open Dev Console" );
	g_devConsole->AddLine( DevConsole::INFO_MAJOR, "Esc: Exit game" );
...
void App::Update() {
	...
	if (!g_window->IsFocus() || g_devConsole->GetMode() == DevConsoleMode::EMERGE) {
...
bool App::SetQuitting( NamedStrings& args )
{
	UNUSED( args );
